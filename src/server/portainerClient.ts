...
/**
 * Find stack using ID or Name env vars
 */
export async function findTargetStack(): Promise<Stack | null> {
  ...
  if (stackName) {
    return getStackByName(stackName);
  }
  
  // Find first stack that matches our repo
  ...
  return stacks.find(s => 
    s.RepositoryURL === repoUrl || 
    s.RepositoryURL === `https://github.com/${owner}/${repo}`
  ) || null;
}